...
public class IntrusionAnalyserV2 extends IRElementVisitor<Integer> {
	...
	public Integer visitEntry(NPEntry e) throws VisitorException {
		...
		// test ip change
		String ipTest = (String) IntrusionAnalyserV2.duplicateIP.get(mac);
		if(ipTest == null){
			IntrusionAnalyserV2.duplicateIP.put(mac, ip);
		}else if(!ipTest.equals(ip)){
			System.out.println(mac + ": changed from " + ipTest + " to " + ip);	
			IntrusionAnalyserV2.duplicateIP.put(mac, ip);
		}
		// test mac change
		String macTest = (String) IntrusionAnalyserV2.duplicateMAC.get(ip);
		if(macTest == null){
			IntrusionAnalyserV2.duplicateMAC.put(ip, mac);
		}else if(!macTest.equals(mac)){
			System.out.println(ip + ": changed from " + macTest + " to " + mac);
			IntrusionAnalyserV2.duplicateMAC.put(ip, mac);
		}
		return null;
	}
	...
	public static void analyse(IR ir) {
		try {
			System.out.println("Starting intrusion detection V2!!!!");
			new IntrusionAnalyserV2().visitEntries(ir.getEntries());
